...
fetch('/api/ideas', {
  ...
  body: JSON.stringify({product, targetAudience, prompt: 'generate a list of listicle ideas for the target market ' + targetAudience }),
})
.then(response => response.json())
.then(({data}:any) => {
  console.log(data)
...
<Tabs>
  ...
  <TabPanels>
    ...
    <Input placeholder="engyne.ai" onChange={(evt:any)=>{
      setURL(evt.target.value)
    }}/>
    <Button onClick={()=>{handleScrape()}} disabled={!url} isLoading={loadingScrape}>Scrape</Button>
    ...
    <Input defaultValue="generate a list of keyword ideas" placeholder="generate a list of keyword ideas" onChange={(evt:any)=>{
      setURL(evt.target.value)
    }}/>

    <Button onClick={()=>{handleGenerateIdeas()}} isLoading={loadingIdeas}>Generate Ideas</Button>
    ...
    <VStack spacing={4} align="flex-start">
    <Button onClick={()=>{handleIngest()}} isLoading={loadingIngest}>Ingest</Button>
        {library.map((item, index)=>(
          <Text key={index} noOfLines={6}>{item.text}</Text>
        ))}
    </VStack>
    </TabPanel>
  </TabPanels>
